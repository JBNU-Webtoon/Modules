...
class Loss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        noobj_target = target_tensor[noobj_mask].view(-1, N)    # target tensor on the cells which do not contain objects. [n_noobj, N]
                                                                # n_noobj: number of the cells which do not contain objects.
        noobj_conf_mask = torch.zeros(noobj_pred.size(), dtype=torch.bool, device='cuda')  # ✅ 변경
        for b in range(B):
            noobj_conf_mask[:, 4 + b*5] = 1 # noobj_conf_mask[:, 4] = 1; noobj_conf_mask[:, 9] = 1
        noobj_pred_conf = noobj_pred[noobj_conf_mask.bool()]  # ✅ 변경       # [n_noobj, 2=len([conf1, conf2])]
        noobj_target_conf = noobj_target[noobj_conf_mask.bool()] # ✅ 변경  # [n_noobj, 2=len([conf1, conf2])]
        loss_noobj = F.mse_loss(noobj_pred_conf, noobj_target_conf, reduction='sum')

        # Compute loss for the cells with objects.
        coord_response_mask = torch.zeros(bbox_target.size(), dtype=torch.bool, device='cuda')  # ✅ 변경  # [n_coord x B, 5]
        coord_not_response_mask = torch.ones(bbox_target.size(), dtype=torch.bool, device='cuda') # ✅ 변경  # [n_coord x B, 5]
        ...
        bbox_pred_response = bbox_pred[coord_response_mask.bool()].view(-1, 5)  # ✅ 변경      # [n_response, 5]
        bbox_target_response = bbox_target[coord_response_mask.bool()].view(-1, 5) # ✅ 변경 # [n_response, 5], only the first 4=(x, y, w, h) are used
        target_iou = bbox_target_iou[coord_response_mask.bool()].view(-1, 5)        # [n_response, 5], only the last 1=(conf,) is used
