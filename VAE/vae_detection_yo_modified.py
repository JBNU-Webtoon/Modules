...
class VAE(nn.Module):
  def __init__(self, input_channels, hidden_dim, latent_dim, feature_size=8, num_bboxes=2, num_classes=2):
    ...
    self.encoder = nn.Sequential(
        ...
        nn.ReLU(),
        nn.Conv2d(512, 45, kernel_size=1)
    )
    self.fc1 = nn.Linear(45*16*16, latent_dim) # 평균
    self.fc2 = nn.Linear(45*16*16, latent_dim) # 분산
    ...
    self.detection_head = nn.Sequential(
        nn.Linear(45 * 16* 16 , 16* 16 * 512), # 차원 맞춰주기용?
        nn.Linear(16 * 16 * 512, 4096), # 여기를 latent dim으로 바꾸느냐 아니면 아래에서 이미지 encoder를 따로 돌리느냐의 차이
