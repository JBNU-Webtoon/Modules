...
class Loss(nn.Module):
    ...
    def __init__(self, feature_size=8, num_bboxes=2, num_classes=2, lambda_coord=5.0, lambda_noobj=0.5):
        """ Constructor.
        Args:
    ...
    def compute_iou(self, bbox1, bbox2):
        ...
        union = torch.clamp(union, min=1e-6)  # 최소값으로 제한
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        noobj_target = target_tensor[noobj_mask].view(-1, N)    # target tensor on the cells which do not contain objects. [n_noobj, N]
                                                                # n_noobj: number of the cells which do not contain objects.
        noobj_conf_mask = torch.zeros(noobj_pred.size(), dtype=torch.bool, device='cuda')
        for b in range(B):
            noobj_conf_mask[:, 4 + b*5] = 1 # noobj_conf_mask[:, 4] = 1; noobj_conf_mask[:, 9] = 1
        noobj_conf_mask = noobj_conf_mask.to(torch.bool)
        noobj_conf_mask = noobj_conf_mask.to(noobj_pred.device)
        ...
        coord_response_mask = torch.zeros(bbox_target.size(), dtype=torch.bool, device='cuda')
        coord_not_response_mask = torch.ones(bbox_target.size(), dtype=torch.bool, device='cuda')
        ...
        coord_response_mask = coord_response_mask.to(bbox_pred.device)
        ...
        coord_response_mask = coord_response_mask.to(bbox_target_iou.device)  # 인덱스 마스크를 같은 GPU로 이동
        target_iou = bbox_target_iou[coord_response_mask].view(-1, 5)        # [n_response, 5], only the last 1=(conf,) is used
        loss_xy = F.mse_loss(bbox_pred_response[:, :2], bbox_target_response[:, :2], reduction='sum')
        loss_wh = F.mse_loss(
            torch.sqrt(torch.clamp(bbox_pred_response[:, 2:4], min=1e-6)),
            torch.sqrt(torch.clamp(bbox_target_response[:, 2:4], min=1e-6)),
            reduction='sum'
        )
        target_iou = target_iou.to(bbox_pred_response.device)
