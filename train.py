...
# Initialize models
vae = VAE(input_channels=3, hidden_dim=400, latent_dim=LATENT_DIM).to(torch.device("cuda:4" if torch.cuda.is_available() else "cpu"))
crnn = CRNN(img_channel=1, img_height=IMG_HEIGHT, img_width=IMG_WIDTH, num_class=NUM_CLASSES).to(torch.device("cuda:4" if torch.cuda.is_available() else "cpu"))
model = CombinedModel(vae, crnn, LATENT_DIM).to(torch.device("cuda:4" if torch.cuda.is_available() else "cpu"))
...
vae_dataset = VOCDataset(
    ...
    label_txt="../Modules/data/bbox.txt"
)
    ...
    for epoch in range(num_epochs):
        ...
        for (vae_data, crnn_data) in zip(vae_loader, crnn_loader):
            ...
            webtoon_image, target_data, bbox_label = (
                webtoon_image.to(torch.device("cuda:4")),
                target_data.to(torch.device("cuda:4")),
                bbox_label.to(torch.device("cuda:4")),
            )

            # CRNN Data (text images)
            text_image, targets, target_lengths = crnn_data
            text_image, targets, target_lengths = (
                text_image.to(torch.device("cuda:4")),
                targets.to(torch.device("cuda:4")),
                target_lengths.to(torch.device("cuda:4")),
            )
