...
from VAE.vae import VAE, loss_function_MSE
...
class CombinedModel(nn.Module):
    def __init__(self, vae, crnn, latent_dim):
        ...
        self.vae = vae
        self.crnn_cnn = crnn.cnn.to('cuda:3')  # CRNN의 CNN 계층
    ...
    def forward(self, webtoon_image, text_image):
        ...
        h = h.to('cuda:3')
        print("h: ", h.shape)
        text_image = text_image.to('cuda:3')
...
vae_dataset = VOCDataset(
    is_train=False,
    input_dir="../Modules/data/Images_with_effects",
    target_dir="/home/jbnu/Images",
    image_dir="../Modules/data/Images_with_effects",
    label_txt="../Moduels/data/bbox.txt"
)
    ...
    for epoch in range(num_epochs):
        ...
        for (vae_data, crnn_data) in zip(vae_loader, crnn_loader):
            ...
            webtoon_image, target_data, bbox_label = (
                webtoon_image.to(torch.device("cuda:1")),
                target_data.to(torch.device("cuda:1")),
                bbox_label.to(torch.device("cuda:1")),
            )

            # CRNN Data (text images)
            text_image, targets, target_lengths = crnn_data
            text_image, targets, target_lengths = (
                text_image.to(torch.device("cuda:3")),
                targets.to(torch.device("cuda:3")),
                target_lengths.to(torch.device("cuda:3")),
            )
            ...
            # Compute losses
            vae_loss = loss_function_MSE(recon, target_data, mu, log_var).to("cuda:3")
            det_loss = detection_loss_fn(detection_output.to("cuda:2"), bbox_label.to("cuda:2")).to("cuda:3")
            # text_output 차원 변경
            ...
            input_lengths = torch.full((batch_size,), sequence_length, dtype=torch.long).to("cuda:3")
            # 크기 출력하여 디버깅
            ...
            ctc_loss = ctc_loss_fn(text_output, targets, input_lengths, target_lengths).to("cuda:3")
